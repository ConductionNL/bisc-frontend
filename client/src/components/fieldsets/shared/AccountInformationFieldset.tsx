            ...
            <Section title={i18n._(t`Accountgegevens`)}>
                <Column spacing={6}>
                    ...
                    <Field label={i18n._(t`Rol`)} horizontal={true}>
                        <Row spacing={1}>
                            {prefillData?.roles.map(role => <LabelTag label={role} color={colorConfig[role]}/>)}
                        </Row>
                    </Field>
                </Column>
            </Section>
        ...
        <Section title={i18n._(t`Accountgegevens`)}>
            <Column spacing={6}>
                ...
                <Field label={i18n._(t`Rol`)} horizontal={true}>
                    <Column spacing={3}>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.coordinator]} defaultChecked={isEqual(prefillData?.roles, [Roles.coordinator])} />
                            <LabelTag color={colorConfig[Roles.coordinator]} label={i18n._(t`Coördinator`)} />
                        </Row>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.mentor]} defaultChecked={isEqual(prefillData?.roles, [Roles.mentor])} />
                            <LabelTag color={colorConfig[Roles.mentor]} label={i18n._(t`Begeleider`)} />
                        </Row>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.coordinator, Roles.mentor]} defaultChecked={isEqual(prefillData?.roles, [Roles.coordinator, Roles.mentor])} />
                            <Row spacing={1}>
                                <LabelTag color={colorConfig[Roles.coordinator]} label={i18n._(t`Coördinator`)} />
                                <LabelTag color={colorConfig[Roles.mentor]} label={i18n._(t`Begeleider`)} />
                            </Row>
                        ...
                        <Row>
                            <RadioButton name={'role'} value={[Roles.volunteer]} defaultChecked={isEqual(prefillData?.roles, [Roles.volunteer])} />

        ...
        <Section title={i18n._(t`Accountgegevens`)}>
            <Column spacing={6}>
                ...
                <Field label={i18n._(t`Rol`)} horizontal={true}>
                    <Column spacing={3}>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.coordinator]} checked={isEqual(prefillData?.roles, [Roles.coordinator])} />
                            <LabelTag color={colorConfig[Roles.coordinator]} label={i18n._(t`Coördinator`)} />
                        </Row>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.mentor]} checked={isEqual(prefillData?.roles, [Roles.mentor])} />
                            <LabelTag color={colorConfig[Roles.mentor]} label={i18n._(t`Begeleider`)} />
                        </Row>
                        <Row>
                            <RadioButton name={'role'} value={[Roles.coordinator, Roles.mentor]} checked={isEqual(prefillData?.roles, [Roles.coordinator, Roles.mentor])} />
                            <Row spacing={1}>
                                <LabelTag color={colorConfig[Roles.coordinator]} label={i18n._(t`Coördinator`)} />
                                <LabelTag color={colorConfig[Roles.mentor]} label={i18n._(t`Begeleider`)} />
                            </Row>
                        ...
                        <Row>
                            <RadioButton name={'role'} value={[Roles.volunteer]} checked={isEqual(prefillData?.roles, [Roles.volunteer])} />

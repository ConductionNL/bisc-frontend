    ...
    return {
        ...
        fetchReport: async (periodFrom: Date, periodTo: Date, organizationId: string) => {
            ...
            await result.refetch({
                ...
                // path: `students.csv?_mapping[Voornaam]=person.givenName&_mapping[Tussenvoegsel]=person.additionalName&_mapping[Achternaam]=person.familyName&_mapping[Aanmaakdatum]=intake.date&_mapping[E-mail%20adres]=person.emails.0.email&_mapping[Telefoon]=person.telephones.0.telephone&_mapping[Aanmeldende%20instantie]=intake.referringOrganization&_mapping[Aanmeldende%20instantie%20Email]=intake.referringPerson.emails.0.email&_mapping[Aanmeldende%20instantie%20Anders]=intake.referringOrganizationOther&_mapping[Hoe%20bij%20Taalhuis%20terecht%20gekomen]=intake.foundVia&_mapping[Hoe%20bij%20Taalhuis%20terecht%20gekomen%20anders%20]=intake.foundViaOther&_mapping[Status]=intake.status&_mapping[Team]=team.name&_mapping[Aanmaakdatum]=@dateCreated&intake.status=ACCEPTED&_dateCreated[from]=${periodFromFormatted}&_dateCreated[till]=${periodToFormatted}&languageHouse._id=${organizationId}`,
                path: `students.csv?_dateCreated[from]=${periodFromFormatted}&_dateCreated[till]=${periodToFormatted}&_mapping[Voornaam]=person.givenName&_mapping[Tussenvoegsel]=person.additionalName&_mapping[Achternaam]=person.familyName&_mapping[Aanmaakdatum]=intake.date&_mapping[E-mail%20adres]=person.emails.0.email&_mapping[Telefoon]=person.telephones.0.telephone&_mapping[Aanmeldende%20instantie]=intake.referringOrganization&_mapping[Aanmeldende%20instantie%20Email]=intake.referringPerson.emails.0.email&_mapping[Aanmeldende%20instantie%20Anders]=intake.referringOrganizationOther&_mapping[Hoe%20bij%20Taalhuis%20terecht%20gekomen]=intake.foundVia&_mapping[Hoe%20bij%20Taalhuis%20terecht%20gekomen%20anders%20]=intake.foundViaOther&_mapping[Status]=intake.status&_mapping[Team]=team.name&_mapping[Aanmaakdatum]=@dateCreated&intake.status=ACCEPTED&languageHouse._id=${organizationId}&fields[]=person.givenName&fields[]=person.additionalName&fields[]=person.familyName&fields[]=intake.date&fields[]=person.emails.email&fields[]=person.telephones.telephone&fields[]=intake.referringOrganization&fields[]=intake.referringPerson.emails.email&fields[]=intake.referringOrganizationOther&fields[]=intake.foundVia&fields[]=intake.foundViaOther&fields[]=intake.status&fields[]=team.name`,
            })
        },
    }
}
